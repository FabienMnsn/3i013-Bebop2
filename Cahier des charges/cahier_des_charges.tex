...
\section{Cinématique des écrans}
 	...
 	\begin{enumerate}
 	\item Lorsque l'utilisateur lance l'application, il arrive sur la page d'accueil (Figure 3). Il y a alors deux sections sur l'écran, l'une proposant d'ajouter un nouveau plan de vol et l'autre proposant d'exécuter un plan de vol parmi ceux existants.\\
	...
	\end{figure}
 	...
 	\item Lors de l'ouverture de la page, la carte est vierge comme le montre la figure 4:
 	%________________
 	\begin{figure}[!ht]
 	\begin{center}
  ...
  En cliquant sur la carte, un marqueur est placé à l'endroit du clic. Chaque marqueur représente un point de passage du drone dans son itinéraire de vol. Les marqueurs peuvent être déplacés à la souris (drag and drop). Lors du survol de la souris sur un marqueur, une bulle indique son numéro d'ordre (0 pour le point de départ).\\
    ...
    \medbreak
 	...
 	%\vspace*{1.5cm}
  	...
  	\item On souhaite retirer des marqueurs, on active le mode de suppression en cliquant sur le switch en bas à gauche (figure 6) et on clique ensuite sur les marqueurs à supprimer. L'ordre des marqueurs et le tracé du trajet s'adaptent :\\
	%________________	
	\begin{figure}[!h]
 	\begin{center}
	...
	%\vspace*{0.5cm}
	\newpage
	\item On clique sur un marqueur pour spécifier l'altitude voulue à ce point (figure 7):
	%\vspace*{1cm}
	%________________
	\begin{figure}[!h]
 	\begin{center}
 	...
 	\item On peut déplacer les marqueurs avec un cliquer-glisser (drag and drop) sur ces derniers (figure 8):
 	%________________
 	\begin{figure}[!h]
 	\begin{center}
